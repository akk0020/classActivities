...
data.forEach((user, index) => {
  console.log(user.address.city, index);
});

// create an array of latitude and longitude coordinate pairs
let coordinates = [];
data.forEach((user, index) => {
  coordinates.push([user.address.geo.lat, user.address.geo.lng], index);
});
...
let motto = [];
data.forEach(user => {
  motto.push(`${user.company.catchPhrase} & ${user.company.bs}`);
});
...
document.querySelectorAll(`.class-week`).forEach(element => {
  element.textContent = "this is the forEach method!";
});
